#!/usr/bin/env python2
# -*- coding: utf-8 -*-

import rospy
from sensor_msgs.msg import Image
from cv_bridge import CvBridge, CvBridgeError
import cv2
import threading
from Queue import Queue  # Python 2'de 'queue' yerine 'Queue' kullanılır
import numpy as np

bridge = CvBridge()

# Her drone için görüntü kuyruğu
image_queue_1 = Queue()
image_queue_2 = Queue()
image_queue_3 = Queue()

def image_callback_1(msg):
    try:
        cv_image = bridge.imgmsg_to_cv2(msg, "bgr8")
        image_queue_1.put(cv_image)
    except CvBridgeError as e:
        rospy.logerr("CvBridge Error: {0}".format(e))

def image_callback_2(msg):
    try:
        cv_image = bridge.imgmsg_to_cv2(msg, "bgr8")
        image_queue_2.put(cv_image)
    except CvBridgeError as e:
        rospy.logerr("CvBridge Error: {0}".format(e))

def image_callback_3(msg):
    try:
        cv_image = bridge.imgmsg_to_cv2(msg, "bgr8")
        image_queue_3.put(cv_image)
    except CvBridgeError as e:
        rospy.logerr("CvBridge Error: {0}".format(e))

def process_images():
    while not rospy.is_shutdown():
        if not image_queue_1.empty():
            cv_image = image_queue_1.get()
            cv2.imshow("Drone 1 Image Window", process(cv_image))
        
        if not image_queue_2.empty():
            cv_image = image_queue_2.get()
            cv2.imshow("Drone 2 Image Window", process(cv_image))
        
        if not image_queue_3.empty():
            cv_image = image_queue_3.get()
            cv2.imshow("Drone 3 Image Window", process(cv_image))
        
        cv2.waitKey(3)

def process(src):
    frame = np.array(src)
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

    hsvFrame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    # Kırmızı renk için iki HSV aralığı tanımlama
    lower_red1 = np.array([0, 120, 70], dtype=np.uint8)
    upper_red1 = np.array([10, 255, 255], dtype=np.uint8)
    
    lower_red2 = np.array([170, 120, 70], dtype=np.uint8)
    upper_red2 = np.array([180, 255, 255], dtype=np.uint8)

    # Maskeleri oluşturma
    mask1 = cv2.inRange(hsvFrame, lower_red1, upper_red1)
    mask2 = cv2.inRange(hsvFrame, lower_red2, upper_red2)

    # İki maskeyi birleştirerek kırmızı rengi tespit etme
    mask = cv2.bitwise_or(mask1, mask2)


    kernel = np.ones((5, 5), "uint8")

    # turuncu renk için maske uygulaması
    mask = cv2.dilate(mask, kernel)
    res_red = cv2.bitwise_and(frame, frame, mask=mask)

    # contur oluşturma ve renk takibi
    _, contours, hierarchy = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    for pic, contour in enumerate(contours):
        area = cv2.contourArea(contour)
        if area > 100:
            x, y, w, h = cv2.boundingRect(contour)
            imageFrame = cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
            return imageFrame

    return frame

def main():
    rospy.init_node('image_listener', anonymous=True)
    rospy.Subscriber("/webcam1/image_raw", Image, image_callback_1)
    rospy.Subscriber("/webcam2/image_raw", Image, image_callback_2)
    rospy.Subscriber("/webcam3/image_raw", Image, image_callback_3)
    
    # Create a separate thread to process OpenCV windows
    threading.Thread(target=process_images).start()

    rospy.spin()

if __name__ == '__main__':
    main()
